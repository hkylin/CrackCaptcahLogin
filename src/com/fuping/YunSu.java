...
public class YunSu{
    ...
    /**
     * 答题
     * @param url 			请求URL，不带参数 如：http://api.ysdm.net/create.xml
     * @param param			请求参数，如：username=test&password=1
     * @param data			图片二进制流
     * @return				平台返回结果XML样式
     * @throws IOException
    ...
    public static String createByUrl(String username, String password,
        ...
        try {
            ...
            CloseableHttpClient httpclient = HttpClients.custom().setSSLContext(sslcontext)
                    .setSSLHostnameVerifier(new MyHostnameVerifier()).setDefaultRequestConfig(requestConfig).setUserAgent("Mozilla/5.0 (Windows NT 6.2; WOW64; rv:47.0) Gecko/20100101 Firefox/47.0").build();
            ...
            result = httpPostImage("http://api.ysdm.net/create.xml", param, data);
    ...
    public static String getResult(String result){
        ...
        try {
            ...
            if(resultNl.getLength() > 0 ) {
                yzmResult = resultNl.item(0).getFirstChild().getNodeValue();
            } else if (errorNl.getLength() > 0) {
                yzmResult=(String.format("错误：%s\r\n", errorNl.item(0).getFirstChild().getNodeValue()));
            } else {
                yzmResult = "未知问题";
            }
